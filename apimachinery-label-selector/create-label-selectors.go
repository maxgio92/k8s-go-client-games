...
func main() {
	...
	supportExists, err := labels.NewRequirement(keySupportBy,
		selection.Exists, []string{})
	if err != nil {
		panic(err)
	}
	supportTeam, err := labels.NewRequirement(keySupportBy,
		selection.In, []string{"team_1", "team_2"})
	if err != nil {
		panic(err)
	}
	partOfReq, err := labels.NewRequirement(keyPartOf, selection.Equals,
